...
if not database_url:
    ...
    print("Asegúrate de haber creado PostgreSQL en Railway")
    sys.exit(1)
...
if not os.path.exists('datos_exportados.json'):
    ...
    print("Los datos deben exportarse primero desde SQL Server")
    sys.exit(1)
...
if datos.get('deudas'):
    ...
    for deuda in datos['deudas']:
        try:
            ...
            print(f"   ⚠️ Error importando deuda {deuda.get('id')}: {e}")
    print(f"   ✅ Deudas importadas")
    print()
...
if datos.get('transacciones'):
    ...
    for trans in datos['transacciones']:
        try:
            ...
            print(f"   ⚠️ Error importando transacción {trans.get('id')}: {e}")
    print(f"   ✅ Transacciones importadas")
    print()
...
if datos.get('perfiles'):
    ...
    for perfil in datos['perfiles']:
        try:
            ...
            print(f"   ⚠️ Error importando perfil {perfil.get('id')}: {e}")
    print(f"   ✅ Perfiles importados")
    print()
...
print("="*60)
print()
print("Las tablas han sido creadas y los datos importados.")
print("Tu aplicación debería funcionar ahora en Railway.")
